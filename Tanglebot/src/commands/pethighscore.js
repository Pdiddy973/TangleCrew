...
async function findPreviousLeaderboardMessages(channel, botUserId) {
  ...
  return [...fetched.values()]
    .filter(m => {
      ...
      // Message 0 leads with the header embed instead of the leaderboard title.
...
async function postLeaderboard(guild, channelId, entries, botUserId) {
  ...
  if (!(prevIds.length > 0 && prevMessages.every(m => m))) {
    console.log('[PHS] Stored leaderboard message ID(s) missing or stale — scanning channel history to recover');
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    if (existing) {
      try {
        ...
        console.error(`[PHS] Could not edit message ${existing.id} (${err.message}), sending new...`);
      }
    }
  ...
  writeJson('pethighscores_message.json', { messageIds: newIds });
  console.log(`[PHS] Leaderboard updated (${newIds.length} message${newIds.length === 1 ? '' : 's'})`);
}
